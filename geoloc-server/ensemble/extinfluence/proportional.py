...
"""
    ...
    ensemble.extinfluence.proportional
    ~~~~~~~~~~~~~~~~~~~~~

    This module finds the decision tree sensitivity when equal number of training instances
...
def choose_training_data(country_training_data, countries_with_less_instances, num_inst, cnt1):
    ...
    for country in country_training_data:
        ...
        if country == cnt1:
            ...
            num_inst2 = min(num_inst, len(country_training_data[country]))
...
def test_classifier_main(country_training_data, countries_with_more_instances, country, geoloc_cls):
    ...
    for test_instance in test_data_list2:
        num_spawn_threads += 1
        thread1 = threading.Thread(target=util_ensemble_code.test_classifier,
                                   args=(test_instance, test_count_list, geoloc_cls,))
        thread1.daemon = True
        thread1.start()
    ...
    while cur_count <= max_count:
        ...
        for i in range(5):
            ...
            for cnt1 in countries_with_more_instances:
                training_data = choose_training_data(country_training_data, countries_with_less_instances,
                                                     cur_count, cnt1)
                training_data = util_ensemble_code.train_classifier(training_data, cur_count)
                country_acc_dict = test_classifier_main(country_training_data, countries_with_more_instances,
                                                        cnt1, training_data)
        ...
        for cnt3 in country_acc_list_dict:
            ...
            csvwriter = csv.writer(open(os.path.join(configs.system.PROJECT_ROOT, configs.system.DIR_DATA,
                                                     configs.system.ANALYSIS_FOLDER_PROPORTIONAL, w_f_name),
                                        'ab'))
